"""
GenAI-Powered Phishing SOAR Web Application
------------------------------------------------
This Streamlit app provides an interactive interface for analyzing suspicious 
URLs using machine learning models. It loads pre-trained classification and
clustering models to predict whether a URL is malicious, attributes threat 
actor profiles, and generates prescriptive response plans using GenAI providers. 
The sidebar allows users to input URL features, and the main page
displays analysis results, risk contributions, threat attribution, 
and recommended actions for security analysts and end-users.
"""
# app.py
import os
import time
...
from pycaret.clustering import predict_model as predict_clu_model
...
def load_classfication_assets():
    """
    Loads the classification model and its feature importance plot from disk.
    Returns:
        model: The loaded classification model.
        plot: Path to the feature importance plot image file.
    """
...
if not classification_model:
    st.error(
        "Model not found. Please wait for the initial training to complete, or check the " \
        "container logs with `make logs` if the error persists."
    )
    st.stop()
else:
...
if not clustering_model:
    st.error(
        "Clustering model not found. Please wait for the initial training to complete, "
        "or check the container logs with `make logs` if the error persists."
    )
    st.stop()
else:
...
Threat_actor_profiles = {
    "Cluster 0": {
        ...
        "icon": "💸",
        "description": (
            "This threat profile is characterized by high-volume, "
            "financially motivated attacks. Their methods are often noisy, "
            "relying on techniques like URL shortening, IP addresses in URLs, "
            "and abnormal structures to overwhelm standard defenses. "
            "The primary goal is widespread credential theft or financial fraud."
        ),
    },
    "Cluster 1": {
        ...
        "icon": "🌐",
        "description": (
            "This profile represents a highly sophisticated and targeted attacker. "
            "They use subtle, well-crafted techniques, such as valid SSL "
            "certificates combined with deceptive sub-domains "
            "(e.g., `login.microsoft.com-validate.net`). "
            "Their goal is typically espionage, intelligence "
            "gathering, or strategic disruption."
        ),
    },
    "Cluster 2": {
        "name": "Hacktivist",
        "icon": "📢",
        "description": (
            "This profile is driven by political or social motives."
            "Their attacks are often opportunistic and may leverage "
            "current events. "
            "The technical sophistication can vary, but a key indicator"
            "is often the use of politically charged keywords or themes "
            "in the URL itself to lure victims."
        ),
    },
}
...
with st.sidebar:
    ...
    form_values = {
        ...
        "prefix_suffix": st.checkbox(
            "URL has a Prefix/Suffix (e.g.,'-')", value=True
        ),
        ...
        "political_keyword": st.checkbox(
            "URL contains a political keyword", value=False
        ),
    }

    st.divider()
    genai_provider = st.selectbox(
        "Select GenAI Provider", ["Gemini", "OpenAI", "Grok"]
    )
    submitted = st.button(
        "💥 Analyze & Initiate Response",
...
if not submitted:
    ...
    risk_scores = {
        ...
        "Complex Sub-domain": (
            10 if input_dict["having_Sub_Domain"] == 1 else 0
        ),
    ...
    with st.status("Executing SOAR playbook...", expanded=True) as status:
        ...
        if is_malicious:
            st.write(
                "▶️ **Step 3: Threat Attribution ** - Threat actor profiling using Clustering **."
            )
            # use only feature the custering model was trainined on
            clustering_model_features = clustering_model.feature_names_in_
            cluster_prediction = predict_clu_model(
                clustering_model, data=input_data[clustering_model_features]
            )
            cluster_id = cluster_prediction["Cluster"].iloc[0]
            threat_profile_info = Threat_actor_profiles.get(
                f"Cluster {cluster_id}",
                {"name": "Unknown", "description": "No profile matched."},
            )
            threat_profile_name = threat_profile_info["name"]
            st.write(
                "▶️ **Step 3: Threat Attribution Complete ** - \
                    Threat actor profiling using Clustering **."
            )
            time.sleep(1)

            st.write(
                f"▶️ **Step 4: Prescriptive Analytics** - \
                Engaging **{genai_provider}** for action plan."
            )
            try:
                ...
                prescription = None  # pylint: disable=invalid-name
                status.update(
    ...
    with tab1:
        ...
        st.caption(
            "This score represents the model's confidence in its prediction."
        )
    ...
    with tab2:
        ...
        if is_malicious:
            ...
            if profile_info:
                st.info(
                    f"**Attributed Profile: {profile_info['icon']} {profile_info['name']}**"
                )
            ...
            st.info(
                "Threat attribution is only performed on URLs classified as malicious."
            )
    ...
    with tab3:
        ...
        st.caption(
            "A simplified view of which input features contributed most " \
        ...
        if feature_plot:
            ...
            st.image(
                feature_plot,
                caption="This plot shows which features the model found most " \
                "important *overall* during its training.",
            )
    ...
    with tab4:
        ...
        if prescription:
            st.success(
                "A prescriptive response plan has been generated by the AI.",
                icon="🤖",
            )
            st.json(
                prescription, expanded=False
            )  # Show the raw JSON for transparency

            st.write("#### Recommended Actions (for Security Analyst)")
            for i, action in enumerate(
                prescription.get("recommended_actions", []), 1
            ):
            ...
            st.text_area(
                "Draft",
                prescription.get("communication_draft", ""),
                height=150,
            )
        else:
            st.info(
