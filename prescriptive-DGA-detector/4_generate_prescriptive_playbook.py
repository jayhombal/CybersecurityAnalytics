...
async def main():
    ...
    # --- Save playbook to Markdown file ---
    try:
        with open("PLAYBOOK.md", "w", encoding="utf-8") as f:
            f.write("# Incident Response Playbook\n\n")
            f.write("## Alert Context\n")
            f.write(findings + "\n\n")
            f.write("## Prescriptive Playbook\n")
            f.write(playbook + "\n")
        print("\n✅ Playbook has been written to PLAYBOOK.md")
    except Exception as e:
        print(f"⚠️ Could not write playbook to file: {e}")
